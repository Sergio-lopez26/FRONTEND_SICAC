...
;
    ...
    const odontologosFiltrados = odontologos.filter((odonto) => {
        const nombreCompleto = `${odonto.primerNombre} ${odonto.segundoNombre} ${odonto.primerApellido} ${odonto.segundoApellido}`.toLowerCase();
        return (
            nombreCompleto.includes(busqueda.toLowerCase()) ||
            odonto.numeroDocumento.includes(busqueda)
        );
    });

    const eligeRegistro = (odontologo) =>{
        setOdontologoElegido({ ...odontologo}); //Acá se copian los datos del odontologo en el form " ...odontologo"
        setMostrarForm(true);
    };
    ...
    const actualizar = (e) =>{
        ...
        setOdontologos(prevOdontologos => 
            prevOdontologos.map(p => p.id === odontologoElegido.id ? odontologoElegido : p)
        );
    };
    
    const cancelar = () =>{
        setMostrarForm(false);
        setOdontologoElegido(null); // Al vaciar este estado, el form se limpia solito
    }; 
            ...
            <div className="d-flex flex-nowrap justify-content-center gap-4">
                ...
                <div id="formP" className="card shadow p-4" style={{ maxWidth: "600px", flexShrink: 0, marginBottom: "30px" }}>
                    <form id="formOdontologo" onSubmit={actualizar}>
                        ...
                        <select name="idTipoDocumento" id="idTipoDocumento" className="form-select" style={{ border: "2px solid #d0e2ff", borderRadius: "12px" }} value={odontologoElegido.idTipoDocumento || ""} onChange={cambioInput} required>
                        ...
                        <input type="number" name="numeroDocumento" id="numeroDocumento" className="form-control" style={{ border: "2px solid #d0e2ff", borderRadius: "12px" }} value={odontologoElegido.numeroDocumento || ""} onChange={cambioInput} required/>
                        </label>

                        <label htmlFor="correo" className="d-block mb-3">Email
                        <input type="email" name="correo" id="correo" className="form-control" style={{ border: "2px solid #d0e2ff", borderRadius: "12px" }} value={odontologoElegido.correo || ""} onChange={cambioInput} required/>
                        </label>
                        ...
                        <input type="date" name="fechaNacimiento" id="fechaNacimiento" className="form-control" style={{ border: "2px solid #d0e2ff", borderRadius: "12px" }} value={odontologoElegido.fechaNacimiento || ""} onChange={cambioInput} required/>
                        </label>

                        <label htmlFor="primerNombre" className="d-block mb-3">Primer nombre
                        <input type="text" name="primerNombre" id="primerNombre" className="form-control" style={{ border: "2px solid #d0e2ff", borderRadius: "12px" }} value={odontologoElegido.primerNombre || ""} onChange={cambioInput} required/>
                        </label>

                        <label htmlFor="segundoNombre" className="d-block mb-3">Segundo nombre
                        <input type="text" name="segundoNombre" id="segundoNombre" className="form-control" style={{ border: "2px solid #d0e2ff", borderRadius: "12px" }} value={odontologoElegido.segundoNombre || ""} onChange={cambioInput} />
                        </label>

                        <label htmlFor="primerApellido" className="d-block mb-3">Primer apellido
                        <input type="text" name="primerApellido" id="primerApellido" className="form-control" style={{ border: "2px solid #d0e2ff", borderRadius: "12px" }} value={odontologoElegido.primerApellido || ""} onChange={cambioInput} required/>
                        </label>

                        <label htmlFor="segundoApellido" className="d-block mb-3">Segundo apellido
                        <input type="text" name="segundoApellido" id="segundoApellido" className="form-control" style={{ border: "2px solid #d0e2ff", borderRadius: "12px" }} value={odontologoElegido.segundoApellido || ""} onChange={cambioInput} />
                        </label>

                        <label htmlFor="numeroCelular" className="d-block mb-3">Número de Celular
                        <input type="tel" name="numeroCelular" id="numeroCelular" className="form-control" pattern="3[0-9]{9}" placeholder="3xxxxxxxxx" style={{ border: "2px solid #d0e2ff", borderRadius: "12px" }} value={odontologoElegido.numeroCelular || ""} onChange={cambioInput} required/>
                        </label>
                        
                        <label htmlFor="tipoSangre" className="d-block mb-3">Tipo de sangre
                        <select name="tipoSangre" id="tipoSangre" className="form-select" style={{ border: "2px solid #d0e2ff", borderRadius: "12px" }} value={odontologoElegido.tipoSangre || ""} onChange={cambioInput} required>
                        ...
                        <input type="text" name="nombreAcudiente" id="nombreAcudiente" className="form-control" style={{ border: "2px solid #d0e2ff", borderRadius: "12px" }} value={odontologoElegido.nombreAcudiente || ""} onChange={cambioInput} />
                        </label>

                        <label htmlFor="documentoAcudiente" className="d-block mb-3">Documento del acudiente
                        <input type="text" name="documentoAcudiente" id="documentoAcudiente" className="form-control" style={{ border: "2px solid #d0e2ff", borderRadius: "12px" }} value={odontologoElegido.documentoAcudiente || ""} onChange={cambioInput} />
                        </label>

                        <label htmlFor="estadoOdontologo" className="d-block mb-3">Estado
                        <input type="text" name="estadoOdontologo" id="estadoOdontologo" className="form-control" style={{ border: "2px solid #d0e2ff", borderRadius: "12px" }} value={odontologoElegido.estadoOdontologo || ""} onChange={cambioInput} />
                        </label>

                        <label htmlFor="fechaRegistro" className="d-block mb-3">Fecha de registro
                        <input type="date" name="fechaRegistro" id="fechaRegistro" className="form-control" style={{ border: "2px solid #d0e2ff", borderRadius: "12px" }} value={odontologoElegido.fechaRegistro || ""} onChange={cambioInput} required/>
                        </label>
                            ...
                            <tbody>
                                {odontologosFiltrados.length > 0 ? (
                                    odontologosFiltrados.map((odonto) => (
                                        <tr key={odonto.id}>
                                            <td>
                                                {odonto.idTipoDocumento}
                                            </td>
                                            <td><strong>{odonto.numeroDocumento}</strong></td>
                                            <td>
                                                {odonto.correo}
                                            </td>
                                            <td>
                                                {odonto.primerNombre} {odonto.segundoNombre} {odonto.primerApellido} {odonto.segundoApellido}
                                            </td>
                                            <td>
                                                {odonto.numeroCelular}
                                            </td>
                                            <td>
                                                <span className={`badge ${odonto.estadoOdontologo === "Activo" ? "bg-primary" : "bg-danger"}`}>
                                                    {odonto.estadoOdontologo}
                                                </span>
                                            </td>
                                            <td>
                                                {odonto.fechaRegistro}
                                            </td>
                                            <td>
